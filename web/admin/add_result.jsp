<%-- 
    Document   : add_result
    Created on : Jun 26, 2017, 3:35:01 PM
    ...
    <body>
        ...
        <c:choose>
            <c:when test="${sessionScope.flag}">
                ...
                <c:set var="message" value="You successfully added a new user." />
            </c:when>
            <c:otherwise>
                <c:set var="result" value="FAILED!" />
                <c:set var="message" value="The user could not be added due to some error. Sorry about that." />
            </c:otherwise>
        </c:choose>
        ...
        <section class="content">
            <h1>${result}</h1>
            <p>${message}</p>
            <a href="/ongc_drms/admin/admin_dashboard.jsp">Go Back</a>
        </section>
    </body>
</html>
